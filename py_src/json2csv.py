...
from betfair_data import MarketImage, Sources, Market, PriceSize
...
def parse_traded(traded: List[PriceSize]) -> Tuple[float, float, float, float]:
    ...
    (wavg_sum, matched, min_price, max_price) = reduce(
        lambda total, ps: (
...
def filter_market(market: MarketImage) -> bool: 
    return (market.country_code == 'AU' 
        and market.market_type == 'WIN' 
        and (c := split_anz_horse_market_name(market.market_name)[2]) != 'trot' and c != 'pace')
...
with open("output.csv", "w") as output:
    ...
    for i, market in enumerate(Sources(market_paths)):
        print("Market {}".format(i), end='\r')

        if not filter_market(market):
            continue

        def get_pre_post_final() -> Tuple[MarketImage, MarketImage, MarketImage]:
            prev_market = None
            preplay_market = None
            postplay_market = None       

            while market.update():
                market_book = market.copy()

                # final market view before market goes in play
                if prev_market is not None and prev_market.in_play != market_book.in_play:
        ...
        if postplay_market is None:
            print(market.market_id + ' has no price data')
            continue; 
        ...
        if preplay_traded is not None:
            def runner_vals(r) -> Dict[str, str]:
        ...
        for (runner, rprices) in zip(market.runners, runner_traded):
            # defining data to go in each column
            output.write(
                "{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{}\n".format(
                    postplay_market.market_id,
                    postplay_market.market_time.astimezone(datetime.timezone.utc).strftime("%Y-%m-%d %H:%M:%S"),
                    postplay_market.country_code,
                    postplay_market.venue,
                    postplay_market.market_name,
                    runner.selection_id,
                    runner.name,
                    runner.status,
                    as_str(runner.sp.actual_sp),
                    rprices['preplay_min'],
                    rprices['preplay_max'],
